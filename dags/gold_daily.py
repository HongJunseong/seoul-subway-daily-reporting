...
with DAG(
    ...
    schedule=None,  # ✅ 필요할 때만 실행 (원하면 나중에 DAILY로 바꾸면 됨)
    catchup=False,
    max_active_runs=1,
    tags=["gold"],
) as dag:
    ...
    gold_arrival = BashOperator(
        ...
        spark-submit --packages "{SPARK_PACKAGES}" \
          src/processing/build_gold_subway_arrival_spark.py \
          "{{{{ ti.xcom_pull(task_ids='compute_target_ymd') }}}}"
        """,
    )

    gold_position = BashOperator(
        task_id="gold_position",
        bash_command=f"""
        set -euo pipefail
        cd {PROJECT_DIR}
        spark-submit --packages "{SPARK_PACKAGES}" \
          src/processing/build_gold_subway_position_spark.py \
          "{{{{ ti.xcom_pull(task_ids='compute_target_ymd') }}}}"
        """,
    )
    ...
    # ✅ 핵심: gold 끝나면 report_daily를 자동 실행
    ...
    target_ymd >> gold_arrival >> gold_position >> emit_gold_ready >> trigger_report_daily
