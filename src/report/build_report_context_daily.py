...
def load_position_line_hourly_gold(target_ymd: str) -> pd.DataFrame:
    ...
    prefix = f"gold/subway_position/line_hourly/year={y}/month={m}/day={d}/"
    print(f"[INFO] Load position GOLD from s3://{bucket}/{prefix}")
...
def summarize_position(df_pos: pd.DataFrame) -> Dict[str, Any]:
    ...
    line_overall = (
        df.groupby(["line_id", "line_name"])
        .agg(avg_active_trains=("active_trains", "mean"), max_active_trains=("active_trains", "max"))
        .reset_index()
        .to_dict(orient="records")
    )
    ...
    if not df_peak.empty:
        peak_stats = (
            df_peak.groupby(["line_id", "line_name"])
            .agg(avg_active_trains_peak=("active_trains", "mean"), max_active_trains_peak=("active_trains", "max"))
            .reset_index()
            .to_dict(orient="records")
        )
    ...
    if "updn_line" in df.columns:
        asym = (
            df.groupby(["line_id", "line_name", "updn_line"])["active_trains"]
            .mean()
            .reset_index()
        )
        ...
        asymmetry = pivot[["line_id", "line_name", "up_active", "down_active", "direction_gap"]].to_dict(orient="records")
...
def build_report_context(target_ymd: str) -> Dict[str, Any]:
    ...
    print(f"[INFO] LAG: usage={USAGE_LAG_DAYS}, arrival={ARRIVAL_LAG_DAYS}, position={POSITION_LAG_DAYS}")
